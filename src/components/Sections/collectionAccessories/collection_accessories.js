...
const CollectionAccessories = () => {
    ...
    const resData = data?.data?.collection_accessories ? Object.values(data.data.collection_accessories) : [];
    ...
    const favouriteHandler = async (productId) => {
        ...
        if (loading) return; // جلوگیری از کلیک‌های مکرر
        ...
        <section className="special-section w-full py-5">
            ...
            <div className='container px-4 flex flex-col md:flex-row justify-center'>
                <div className="flex justify-center flex-col items-center flex-initial md:w-[25%]">
                    <img src="https://mrh-store.com/static/media/accessories.2ef9f5bb.png" alt="تخفیفات ویژه مستر اچ" className="w-full max-w-[325px] self-center section-img" />
                    <h2 className='text-[#31eee8] text-center text-[1.2rem] font-bold cursor-pointer'>اکسسوری و لوازم کلکسیونی</h2>
                </div>
...
export default CollectionAccessories;
