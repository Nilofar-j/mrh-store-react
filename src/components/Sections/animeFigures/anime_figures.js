...
const AnimeFigures = () => {
    ...
    const resData = data?.data?.anime_figures ? Object.values(data.data.anime_figures) : [];
        ...
        try {
            ...
            if (response.status === 200) {
                const updatedFavourites = {
                    ...favouriteStatus,
                    [productId]: !favouriteStatus[productId],
                };
        ...
        <section className="special-section w-full py-5">
            ...
            <div className='container px-4 flex flex-col md:flex-row justify-center'>
                <div className="flex justify-center flex-col items-center flex-initial md:w-[25%]">
                    <img src="https://mrh-store.com/static/media/anime.4d0a3171.png" alt="تخفیفات ویژه مستر اچ" className="w-full max-w-[325px] self-center section-img" />
                    <h2 className='text-[#31eee8] text-center text-[1.2rem] font-bold cursor-pointer'>خرید فیگور شخصیت های انیمه</h2>
                </div>
                            ...
                            {resData.map((discountItem, index) => (
                                <SwiperSlide key={discountItem.product_id}>
                                    <div className="product-card card bg-[#201c2a] pb-4">
                                        <div className="image-wrapper flex items-center justify-center overflow-hidden">
