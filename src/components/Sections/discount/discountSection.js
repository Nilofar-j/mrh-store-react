...
const DiscountSection = () => {
    ...
    const resData = data?.data?.special_offer_products ? Object.values(data.data.special_offer_products) : [];
        ...
        <section className="special-section w-full py-5">
            ...
            <div className='container px-4 flex flex-col md:flex-row justify-center'>
                <div className="flex justify-center flex-col items-center flex-initial md:w-[25%]">
                    <img src="https://mrh-store.com/static/media/discount.d4d9a01c.webp" alt="تخفیفات ویژه مستر اچ" className="w-full max-w-[325px] self-center section-img" />
                </div>
                            ...
                            {resData.map((discountItem, index) => (
                                <SwiperSlide key={discountItem.product_id}>
                                    <div className="product-card card bg-[#201c2a] pb-4">
                                        <div className="MuiChip-root absolute top-0 left-0 h-[30px] w-[32px] z-10 bg-[#171420] text-[#31eee8] text-nowrap rounded-br-md rounded-tl-md">
                                            <span className="MuiChip-label text-[17px]">۱۰%</span>
                                        </div>
                                        ...
                                        <div className="card-body flex flex-col items-center justify-around h-[145px] text-center">
                                            <h3 className="card-title text-center mb-0">{discountItem.name}</h3>
                                                ...
                                                {discountItem.features.filter(fitureProduct => fitureProduct.product_id === discountItem.product_id)
                                                    .map(fitureProduct => (
                                                        <div key={fitureProduct.id} className="price flex flex-col flex-wrap items-end w-fit">
                                                            < p className="text-toman offer line-through text-smaller flex justify-center card-text" >
                                                                <span>{Number(fitureProduct.price).toLocaleString('fa-IR')} </span>تومان
                                                            </p>
                                                            <p className="text-toman text-bold flex mt-1 justify-center card-text">
                                                                <span>{Number(fitureProduct.final_price).toLocaleString('fa-IR')} </span>تومان
                                                            </p>
                                                        </div>
                                                    ))
                                                }
