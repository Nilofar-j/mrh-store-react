        ...
        const fetchFavorites = async () => {
            try {
                ...
                const response = await axios.get(
                    "https://api.mrh-store.com/api/customer/dashboard/favorites",
                    {
                        headers: {
            ...
            <div class="app-content content">
                ...
                <div className="content-wrapper">
                    <div className="grid grid-cols-2 gap-8">
                        <div className="card bg-[#201c2a]  text-[#31eee8] py-3 px-4 text-[.9rem]">
                            <div className="card-header flex items-center justify-between">
                                <span>سفارشات باز</span>
                                <span><Link to="/customer/orders">نمایش همه</Link></span>
                            </div>
                            <div className="card-body p-4 pb-3">
                                {cart.length > 0 ? (
                                    cart.map((productCart) => {
                                        ...
                                        return (
                                            <div key={productCart.product_id} class="box-product border border-[#31eee8] rounded-lg flex flex-row my-5 p-2">
                                                <div class="image-wrapper me-4">
                                                    {productCart.images.filter((image) => image.is_main === "1")
                                                        .map((image) => {
                                                            return <img key={image.image_id} src={image.image_link} alt="Product" className="rounded-md shadow-md max-w-none w-[130px] h-[130px] object-cover" />
                                                        })}
                                                </div>
                                                <div class="detail-item w-[80%]">
                                                    <div class="detail-product w-full">
                                                        <div class="name-price flex flex-col gap-4 ">
                                                            <span>{productCart.name}</span>
                                                            <div class="price self-end">
                                                                {
                                                                    productCart.features.map((productPrice) => {
                                                                        return <p className="font-bold card-text text-[#cbcacb] me-4">{Number(productPrice.price).toLocaleString('fa-IR')}</p>
                                                                    })}
                                                            </div>
                                                        </div>
                                                    </div>
                        ...
                        <div className="card bg-[#201c2a]  text-[#31eee8] py-3 px-4 text-[.9rem]">
                            <div className="card-header flex items-center justify-between">
                                ...
                                <span>
                                    <Link to="/customer/favorite">نمایش همه</Link>
                                </span>
                            </div>
                            <div className="card-body p-4 pb-3">
                                {favorite && favorite.length > 0 ? (
                                    favorite.map((favoriteItems) => (
                                        <a href="#" className="flex flex-row border-b border-b-[#ededed] py-5" key={favoriteItems.product.id} >
                                            <div className="item-img text-center">
                                                {favoriteItems.product.images.filter(image => image.is_main === "1")
                                                    .map((imageItem) => {
                                                        return <img key={imageItem.image_id} className="favorite-customer-dashboard-image h-auto" src={imageItem.image_link} alt="favorite" />
                                                    })
                                                }
                                            </div>
                                            <div className="my-auto mx-5">
                                                {favoriteItems.product.name}
                                                <div className="my-auto pt-2"></div>
                                            </div>
                                        </a>
                                    ))
                                ) : (
